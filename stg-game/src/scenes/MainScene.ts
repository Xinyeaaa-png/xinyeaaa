  ...
  private checkCollisions(): void {
    ...
    for (const bullet of playerBullets) {
      for (const monster of monsters) {
        ...
        const hitRadius = bullet.getRadius() + monster.displayWidth / 2; // Approximation, monster.displayWidth/2 assumes a square monster
        if (dist < hitRadius) {
          ...
          break; // Bullet hit one monster, break to avoid hitting other monsters
        }
      }
    }
